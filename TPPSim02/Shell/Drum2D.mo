...
//Паровое пространство барабана
  ...
  Q_top = hfrDrumTop(S_top = S_top, t_m_steam = t_m_steam, ts = ts, D_st_circ = D_st_circ, D_st_eco = D_st_eco, Dvipar = Dvipar, h_dew = h_dew, h_bubble = h_bubble);
...
//  tw = Medium.saturationTemperature(pw);
  ...
  Q_bot = hfrDrumBot(S_bot = S_bot, T = state_w.T, t_m_water = t_m_water);
