...
// Параметры уравнений динамики
  ...
  Pipes.VolumeNode[numberOfFlueSections, numberOfTubeSections] node(each deltaVFlow = deltaVFlow,
                                                                    ...
                                                                    p_start = if numberOfTubeSections == 1 then fill(fill((pin_start + pout_start) / 2, numberOfTubeSections), numberOfFlueSections)
                                                                              else fill(linspace(pin_start, pout_start, numberOfTubeSections), numberOfFlueSections))  annotation(
...
equation
  for i in 1:numberOfFlueSections loop
    for j in 2:numberOfTubeSections loop
      ...
      connect(channel[i, j-1].Output, node[i, j].Port[1]);
      connect(node[i, j].Port[2], channel[i, j].Input);

    end for;
  end for;
  
  for i in 1:numberOfFlueSections loop
    connect(channel_fix_flow[i].Output, node[i, 1].Port[1]);
    connect(node[i, 1].Port[2], channel[i, 1].Input);
  end for;  //Вход и выход
  for i in 1:numberOfFlueSections loop
