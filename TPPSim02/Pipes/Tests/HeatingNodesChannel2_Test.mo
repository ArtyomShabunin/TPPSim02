...
model HeatingNodesChannel2_Test
  ...
  TPPSim02.Thermal.TubeWall[numberOfVolumes+1] wall(each L = 1 * 10, each Tstart1 = 40 + 273.15, each TstartN = 40 + 273.15, each lambda = 20, each rext = 0.301 / 2, each rhomcm = 7800 * 650, each rint = 0.3 / 2) annotation(
    Placement(visible = true, transformation(origin = {-10, 10}, extent = {{-10, 10}, {10, -10}}, rotation = 0)));
  Modelica.Thermal.HeatTransfer.Sources.FixedHeatFlow[numberOfVolumes+1] fixedHeatFlow(each Q_flow = 0) annotation(
  ...
  TPPSim02.Pipes.HeatingNodesChannel2 channel(numberOfVolumes = numberOfVolumes)  annotation(
...
equation
  ...
  connect(channel.heat, wall.int) annotation(
    Line(points = {{-10, -26}, {-10, 8}}, color = {191, 0, 0}, thickness = 0.5));
  annotation(
    experiment(StartTime = 0, StopTime = 1000, Tolerance = 1e-03, Interval = 0.2));

end HeatingNodesChannel2_Test;
